...
- [db-declarative-schema](rules/db-declarative-schema.md) - 宣言的スキーマ運用（schemas/ + db diff）

### 2. Edge Functions (HIGH)

- [edge-template](rules/edge-template.md) - `handler()` ひな形
- [edge-auth](rules/edge-auth.md) - `getClaims` / `requireAdmin`
- [edge-exceptions](rules/edge-exceptions.md) - webhook / cron / health
...
- [logging-required](rules/logging-required.md) - 必須ログ（handler 内包）
- [logging-structure](rules/logging-structure.md) - `createRequestLogger`
...
`backend/supabase/schemas/` を編集 → `supabase db diff` → `npm run sb:types:gen`
...
```
Page / Mobile → Edge Function（handler） → Database Function / service_role オーケストレーション → DB
```

PostgreSQL 17 / Deno 2（`config.toml`）。
