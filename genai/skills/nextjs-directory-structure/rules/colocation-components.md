...
データ取得コンポーネントにスケルトンが必要な場合は、同ディレクトリに `{Name}Skeleton.tsx` を置く。実装は shadcn/ui の `Skeleton`（`@/components/ui/skeleton`）を使う。詳細は `nextjs-library-guide` の `ui-components` を参照。
