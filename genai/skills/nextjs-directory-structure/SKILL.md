...
- [colocation-utils](rules/colocation-utils.md) - ヘルパー関数（_utils/）
...
| 用途           | 単一ページ / 機能       | 共通（任意）        |
|----------------|-------------------------|-------------------|
| Server Actions（書き込み） | `_actions/[resource].ts` + `schema.ts` / `types.ts` | - |
| API（読み取り）| `_apis/[resource].server.ts` | `apis/` は任意・現状未使用 |
| コンポーネント | `_components/Name/`     | `components/ui/*.tsx`（shadcn フラット） |
| ヘルパー関数   | `_utils/[name].ts`（kebab-case） | `utils/[name].ts` |

モノレポパスは `frontend/web/`、E2E は `frontend/web/test/e2e/`。
