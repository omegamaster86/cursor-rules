...
- [ ] `Deno.env.get()` にfallback値（`?? "default"` や `|| "default"`）を設定していない
- [ ] 環境変数が未設定の場合にエラーをスローする仕組みがある（例: `if (!value) throw new Error(...)`）
...
- [ ] POST/PATCH/DELETE を含む API は許可メソッドを明示
- [ ] PUT は許可していない（更新は PATCH で部分更新とする）

### HTTPメソッドの使い分け

| メソッド | 用途 | レビュー観点 |
|----------|------|--------------|
| GET | データ取得 | 一覧・単体・検索・エクスポートなど読み取り専用。クエリパラメータで条件を渡す |
| POST | データ作成 | 新規登録・申込・送信など、冪等でない操作やリソース作成 |
| PUT | **禁止** | 許可しない。全体更新は使わず PATCH で部分更新する |
| PATCH | データ更新（部分） | 変更フィールドのみ送る。プロフィール・設定・ステータス変更など |
| DELETE | データ削除 | リソース削除。論理削除の場合は「削除リクエストを送る」役割 |

- [ ] 取得系 API は GET のみ許可している
- [ ] 作成系 API は POST のみ許可している
- [ ] 更新系 API は PATCH を許可し、PUT は許可していない
- [ ] 削除系 API は DELETE のみ許可している
