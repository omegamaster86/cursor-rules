...
| Cloud 不可、またはローカル Agent（**本 PJ はこちらがデフォルト**） | **D. LOCAL_AUTO** |
| QuickTime 等を人間が手動で回す場合のみ | B. 手動 OS 録画 + browser MCP |

**本 PJ では D. LOCAL_AUTO をデフォルトとする。** A が使えない理由をユーザーに確認する必要はない。
...
#### A. Cursor Cloud Agent（Cloud 利用可の PJ のみ）
...
#### D. LOCAL_AUTO（**本 PJ デフォルト / Cloud 不可 PJ**）
...
1. Doctor（本 PJ 前提の確認）
2. **D. LOCAL_AUTO**（本 PJ）または A. Cloud で録画 + browser 操作
...
- Cloud 不可 PJ でも LOCAL_AUTO + `gh --attach` で PR 検証動画運用は同等に可能
