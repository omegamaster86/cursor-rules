...
todolist を開き、最初の項目は **poteto-mode** スキルの Principles 節を読むこと。続けて下記フェーズを todo に追加する。

## フェーズ A: 枠組み

まず土台を固め、からコミットする。次が言えるまで実行を始めない:

- 完了の定義を反証可能な述語として（**prove-it-works** 原則スキル）。「うまく終わった」は検証可能でなければならない。
...
原子的で独立して着地できるユニットに分解する。リスクの高い未知を先に並べ、オプション価値を高く保つ。足場と検証を機能の前に（**foundational-thinking** 原則スキル）。

- 作業の前に検証ハーネスを組み、変更前状態からベースラインを取得し、チェックが「旧値 vs 新値」として読めるようにする。
- 一方通行の設計判断には **architect** スキル（内部で **arena** を実行）を、多様で隔離され意見の強い候補と、別モデルファミリの読み取り専用ジャッジで走らせる。形が既に具体な機械的作業ではスキップ。固まった設計への 2 回目の arena は過剰設計（**laziness-protocol** 原則スキル）。
- 何を扇状展開するか決める。本当の境界でのみ並列化し、各ワーカーに専用 worktree またはブランチ（**separate-before-serializing-shared-state** 原則スキル）。過剰に扇状展開しない。
