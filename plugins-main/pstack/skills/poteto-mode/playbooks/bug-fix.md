...
2. 原因を binary-search。candidate 仮説を形成し、1つ残るまで排除。`how` で affected subsystem、**why** スキルで regression 履歴から seed。各 pass で残 problem space を最も切る split を取り、runtime evidence を得て eliminate。program state が unclear なら instrumentation または logging を追加し、コード実行中に読む。guess しない。長いまたは stubborn な hunt は Cursor の `/loop` で drive。step-3 architect/interrogate fan-out 前に、surviving *mechanism* を runtime evidence で confirm。
3. fix を plan。function boundary を越えるなら先に `architect`。specific scope で設定 bug-fix model（デフォルト `gpt-5.5-high-fast`）の subagent に implementation を delegate。diff を review。
...
Investigation は `how` + `why` を parallel subagent に fan-out。
