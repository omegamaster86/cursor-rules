...
`poteto-mode` スキルの **Principles** セクションに接地したフェーズ実装プランを produce する。プランが成果物。実装しない。
...
`poteto-mode` スキルの **Principles** セクションを最初から最後まで読み、インデックスする leaf `principle-*` スキルも読む。原則がすべてのプラン決定を統治。cross-link する。
...
- `subagent_type: "poteto-agent"` を優先。`generalPurpose` は fallback。組み込み `plan` subagent_type は使わない。このスキルを無視する。
...
- Browser / Electron / Web UI：`cursor-team-kit` プラグインの `control-ui` スキル。
- CLI と TUI：`cursor-team-kit` プラグインの `control-cli` スキル。
...
overview で、実装者が名前で適用すべき poteto-mode non-negotiables を名指し：

- 変更前に unfamiliar な各サブシステムで **how** スキル。
- 出荷前に contested design で **interrogate** スキルによる敵対的レビュー。
- 各 diff に commit 前 `/deslop`。散文 surface に **unslop** スキル。
