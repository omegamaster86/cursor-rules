---
name: poteto-mode
description: 簡潔で詳細な返信、意図的なサブエージェント、unslopped な散文、シンプルなコード、検証済みの作業のための poteto のエージェントスタイル。poteto、/poteto-mode、またはこのスタイルでの作業リクエストに使用。
...
- 争点のある設計 → 出荷前に **interrogate** スキル（4モデル敵対的）。
- 非自明な複数ステップ → throughput checkpoint を書く（Feature ステップ 3）。
- 散文の表面がある → **unslop** スキル。返信は散文の表面。 **Writing the reply** に従って書く。エージェント向け散文も **create-skill** スキル（SKILL.md 作成用の Cursor 組み込み）に従う。
- コミット前 → `cursor-team-kit` プラグインの `deslop` スキル（`/deslop`）。
- UI / IDE / CLI を出荷 → 対応する control スキル。`cursor-team-kit` は `control-cli`（CLI と TUI）と `control-ui`（ブラウザ / Electron / web UI）を公開。バグ修正では、同じ表面で先に自分で再現。Bug fix ステップ 1 の狭い例外でのみユーザーに渡す。
...
**プレイブックステップ内で spawn するサブエージェントはすべて `subagent_type: "poteto-agent"` を使う**（コード書き delegate、ad-hoc ヘルパー）。`/poteto-mode` と `poteto-agent` は同じラッパーを通る。ルーティングされたワークフロースキル（`how`、`why`、`interrogate`、`reflect`）は diverse-model レビュー用に独自の `subagent_type` を設定。スキルが規定するものを尊重し、`poteto-agent` で上書きしない。

**すべての `Task` 呼び出しのデフォルト。** `run_in_background: true`、agent mode（readonly は MCP を strip）、インライン context ではなく file pointer、ロールごとの明示的 model（`/setup-pstack` で設定可能。デフォルト code は `composer-2.5-fast`、散文と judgment は `claude-opus-4-8-thinking-xhigh`）。
...
下書き時から返信をクリーンに書く。後からの cleanup パスは失敗することが計測されているので、最初から悪い文を生成しない。

- **短い宣言文。** 1文1思考、ピリオドで終える。
- **長ダッシュ文字は完全禁止。** 2ケース。ファイル名と説明をダッシュでつなぐファイルリスト bullet。文として書く（「`main.js` が persistence と IPC handler を所有する」）。bold セクションヘッダーと本文をダッシュでつなぐケース。ヘッダーを独立した文に（「**Verification.** CDP 経由で end to end」）。
- **文中コネクタとしてのコロンも禁止**（unslop ルール 14）。リスト前のコロンは可。
- **簡潔は内容省略の言い訳にならない。** 短い文だが、プレイブックの返信が名指しする各セクションは残す：詳細、トレードオフ、選択、未決定事項。
- **コンシューマーとメンテナ向けに impact をフレーム。** 誰のための作業か（エンドユーザー、ライブラリを import する同僚）と、実装詳細の前に彼らに何が変わるか。次にこのコードを所有するエンジニアが何を引き継ぐか。どちらも気づくことが言えなければ、作業か説明がずれている。
- **リンク、引用、トランスクリプト参照を捏造しない。** このセッションで produce または read したアーティファクトだけリンク。

すべてのプレイブックはこの方法で書いた返信で終わる。PR リンクは `https://github.com/<owner>/<repo>/pull/<number>`。以下のプレイブック別行はそのプレイブック固有の内容のみ名指し。

## Comments

コメントも返信と同じルール。書きながらクリーンに。平坦な「叙述コメント禁止」だけでは捕まえられない。最初から書かない。繰り返し捕まえるケースは、フェーズを叙述する verify または test script、`// Phase 1: add cards` のようなブロック上の行。削除。assertion または log 文字列だけが必要な doc。`// move the card` コメント＋コードではなく `assert(ok, 'persisted across restart')` を書く。delegate の diff と verify script を含め produce するすべてのファイルに適用。コードが示せない非自明な *why* だけコメントを残す。
