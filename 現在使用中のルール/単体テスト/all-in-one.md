...
### 画面遷移（Next.js App Router）の単体テスト例
- `next/navigation` をモックしてルーター操作と404フォールバックを検証する。
- `@testing-library/react` と `user-event` でユーザー操作を再現する。
- 異常系の404表示は `notFound` をスタブして例外を抑止し、フォールバックUIの描画を確認する。
- サンプルコードは `./test-sample-code.md` の「Next.js画面遷移テスト」を参照。
