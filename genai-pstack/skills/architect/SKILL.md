...
どちらでも統合は単独コミットとして出荷できる。**foundational-thinking** 原則スキルの「scaffold first」モード。以降のコミットは安定契約に対する本体の充填として読める。充填中の計画・スコープ内の破壊は **outcome-oriented-execution** 原則スキルに従い問題ない。実装前に設計へ敵対的圧力をかけるなら、統合スケッチに **`review-orchestrator-triple-hybrid` コマンド**（または `interrogate` スキル）を実行。
