...
3. throughput checkpoint を4 todo item として書く。genuinely apply しない dimension（single file、no fan-out）は item を `n/a: <reason>` で keep。drop しない：
   ...
   - **Shared mutable state.** default は target split（専用ファイル・ブランチ・キー）。real invariant のみ serialize。
...
6. matching surface で verify。**`/verify-done`**（Tier C/D）。Inconclusive または wrong-surface は pass ではない。flag。
