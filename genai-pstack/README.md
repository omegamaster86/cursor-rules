...
- **omega-mode**: `/omega-mode` コマンド起動時、`commands/omega-mode.md` と `skills/omega-mode/` が `global.mdc` より優先。原則12本 + プレイブック + 検証重視。完了前検証は **`/verify-done`** が正本。検証・層配線は genai ドメインスキル、リファクタ調査は `/refactor-check` が正本。
